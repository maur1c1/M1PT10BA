...
function buildToDo(todo, index) { //{ description: '', complete: false}
  // Tu código acá:
  let toDoShell = document.createElement('div'); //<span id = 1> hola </span>
...
function buildToDos(toDos) { // [{description: '', complete: false}, {description: '', complete: false}, {description: '', complete: false}]
  // Tu código acá:
let array = toDos.map(function(todo, index){
  return buildToDo(todo, index)
})
  
return array; //[<div> <span id> Hola</span> </div>, <div> <span> </span> </div>,<div> <span> </span> </div>,..]
...
function displayToDos() {
  ...
  //<div id="toDoContainer"> <div> <span id> Hola</span> </div>, <div> <span> </span> </div>,<div> <span> </span> </div> </div>
  // result.map(todo => toDoContainer.appendChild(todo)) || buildToDos(toDoItems).map(todo => toDoConatiner.appendChild(todo))
...
function addToDo() {
 ...
 if(input.value !== ''){
    let todo = new ToDo(input.value) //{description: "", complete:false}
    toDoItems.push(todo);
    input.value ="";
    displayToDos();
 }
